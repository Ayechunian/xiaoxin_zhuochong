...
新素材在 `assets/poses/` 的 `edge_left.png`、`edge_right.png`、`edge_top.png` 和 `edge_bottom.png`，每个方向一张透明 PNG。按用户提供的角色风格校准肤色与细线风格，去掉额外腮红；走路动作使用 `assets/sheet_animations/right_walk/` 和 `left_walk/` 中左右各八帧的原始提取素材。
